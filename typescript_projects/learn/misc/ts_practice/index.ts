...
setTimeout(() => {
  console.log("asdasdxxxx");
}, 4000);

let numbers = [1, 2, 3, 4, 5, 77, 33];
let multipliednumbers = numbers.map((i) => {
  return i * 2;
});
...
let arrTest = [5, 4, 3, 7, 15];

console.log(
  arrTest.filter((element) => {
    if (element > 6) {
      return element;
    }
  }),
