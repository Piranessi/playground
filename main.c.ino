#include <Thermocouple.h>
#include <MAX6675_Thermocouple.h>
#include <SmoothThermocouple.h>
#include "BluetoothSerial.h"

#if !defined(CONFIG_BT_ENABLED) || !defined(CONFIG_BLUEDROID_ENABLED)
#error Bluetooth is not enabled! Please run `make menuconfig` to enable it
#endif

// --- Ustawienia systemowe ---
#define SMOOTHING_FACTOR 1
#define ERROR_LIMIT 15
#define TEMPERATURE_SENSORS_SKIP_IN_MS 1

// --- Bluetooth ---
BluetoothSerial SerialBT;
String msg_bt = "";

// --- Termopara ---
Thermocouple* thermocouple = nullptr;
int temperature_bottom_current = -1;
int temperature_bottom_current_prev = -1;
int temperature_bottom_target = 50;
int temp_dif_bot_target_current = 0;
int histeresis_coeff = 50;
int error_count = 0;

// --- Piny termopary (MAX6675) ---
const int POWER_SOURCE_PIN = 23;
const int termocouple_bottom_SO = 13;
const int termocouple_bottom_CS = 12;
const int termocouple_bottom_SCK = 14;

// --- Timer i czas ---
volatile int interruptCounter = 0;
hw_timer_t* timer = nullptr;
portMUX_TYPE timerMux = portMUX_INITIALIZER_UNLOCKED;

unsigned long time_from_system_up_in_seconds = 0;
unsigned long stop_time_from_system_up_in_seconds = 0;

// --- Parametry grzania ---
const int very_careful_stage_dif_degrees = 15;
const int very_careful_heating_time = 6;
const int careful_heating_time = 10;
int cnt_temp_sensor = TEMPERATURE_SENSORS_SKIP_IN_MS;

// --- Funkcje pomocnicze ---
void IRAM_ATTR onTimer() {
    portENTER_CRITICAL_ISR(&timerMux);
    interruptCounter++;
    portEXIT_CRITICAL_ISR(&timerMux);
}

void setup_timer() {
    timer = timerBegin(0, 80, true); // 80MHz / 80 = 1MHz -> 1 tick = 1us
    timerAttachInterrupt(timer, &onTimer, true);
    timerAlarmWrite(timer, 1000000, true); // 1s
    timerAlarmEnable(timer);
}

void task_calculate_time() {
    if (interruptCounter > 0) {
        portENTER_CRITICAL(&timerMux);
        interruptCounter--;
        portEXIT_CRITICAL(&timerMux);
        ++time_from_system_up_in_seconds;
    }
}

String time_sec_tohhmmss(unsigned long sec) {
    char buffer[16];
    snprintf(buffer, sizeof(buffer), "%02lu:%02lu:%02lu", sec / 3600, (sec % 3600) / 60, sec % 60);
    return String(buffer);
}

void errorOccured() {
    ++error_count;
    SerialBT.println("Error occurred. Count = " + String(error_count));
    if (error_count >= ERROR_LIMIT) {
        SerialBT.println("Error limit reached. Restarting ESP...");
        delay(5000);
        ESP.restart();
    }
}

void sensor_temperature_bottom() {
    int temp = (int)thermocouple->readCelsius();
    temperature_bottom_current_prev = temperature_bottom_current;

    if (abs(temp - temperature_bottom_current_prev) > histeresis_coeff) {
        errorOccured();
        return;
    }

    temperature_bottom_current = temp;
}

int time_from_last_stop() {
    return (int)(time_from_system_up_in_seconds - stop_time_from_system_up_in_seconds);
}

unsigned int time_left_heating(int duration) {
    int elapsed = time_from_system_up_in_seconds - stop_time_from_system_up_in_seconds;
    return (duration > elapsed) ? (duration - elapsed) : 0;
}

void heating_handler() {
    sensor_temperature_bottom();
    temp_dif_bot_target_current = temperature_bottom_target - temperature_bottom_current;

    if (temperature_bottom_current < temperature_bottom_current_prev) {
        stop_time_from_system_up_in_seconds = time_from_system_up_in_seconds;
    }

    if ((temp_dif_bot_target_current > very_careful_stage_dif_degrees) &&
        (time_from_last_stop() < careful_heating_time)) {

        digitalWrite(POWER_SOURCE_PIN, HIGH);
        SerialBT.println("HEAT=ON careful (" + String(time_left_heating(careful_heating_time)) + "s)");
    }
    else if ((temp_dif_bot_target_current > 1) &&
             (time_from_last_stop() < very_careful_heating_time)) {

        digitalWrite(POWER_SOURCE_PIN, HIGH);
        SerialBT.println("HEAT=ON very careful (" + String(time_left_heating(very_careful_heating_time)) + "s)");
    }
    else {
        digitalWrite(POWER_SOURCE_PIN, LOW);
        SerialBT.println("HEAT=OFF");
    }
}

void task_heat(int time_ms) {
    if (--cnt_temp_sensor <= 0) {
        heating_handler();
        cnt_temp_sensor = TEMPERATURE_SENSORS_SKIP_IN_MS;
    }
}

void task_BT_Handler() {
    while (SerialBT.available()) {
        msg_bt += (char)SerialBT.read();
    }

    if (!msg_bt.isEmpty()) {
        if (msg_bt.startsWith("1")) {
            temperature_bottom_target = msg_bt.substring(1).toInt();
            SerialBT.println("New target = " + String(temperature_bottom_target));
        }
        else {
            SerialBT.println("Invalid command header.");
        }
        msg_bt = "";
    }

    SerialBT.printf("Temp bottom: %d\n", temperature_bottom_current);
    SerialBT.printf("Target: %d\n", temperature_bottom_target);
    SerialBT.println(time_sec_tohhmmss(time_from_system_up_in_seconds));
}

void BT_print_x_lines(unsigned int amount) {
    for (unsigned int i = 0; i < amount; ++i)
        SerialBT.println("");
}

// --- Setup & Loop ---
void setup() {
    Serial.begin(115200);
    setup_timer();

    thermocouple = new SmoothThermocouple(
        new MAX6675_Thermocouple(termocouple_bottom_SCK, termocouple_bottom_CS, termocouple_bottom_SO),
        SMOOTHING_FACTOR
    );

    SerialBT.begin("SzzczZ");
    pinMode(POWER_SOURCE_PIN, OUTPUT);
    digitalWrite(POWER_SOURCE_PIN, LOW);

    delay(2000);
}

void loop() {
    BT_print_x_lines(40);
    task_calculate_time();
    task_heat(TEMPERATURE_SENSORS_SKIP_IN_MS);
    task_BT_Handler();
    delay(1000);
}
